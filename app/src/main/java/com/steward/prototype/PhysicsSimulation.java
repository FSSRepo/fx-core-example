...
public class PhysicsSimulation {
	...
	MainActivity.PrototypeUNI proto;
	
	public PhysicsSimulation(MainActivity.PrototypeUNI proto){
		this.proto = proto;
	}
	...
	private void startPhysicsEngine(){
		...
		if(plCreateContext()){
			Log.i("Prototype","Physics Library Context has been created.\nVersion: "+plGetString(PL_VERSION));
			createDynamicWorld();
		}
	}
	...
	private void createDynamicWorld(){
		...
		Log.i("FISICAS", FX.homeDirectory + "proto/placement.dat" + " " + (new File(FX.homeDirectory + "proto/placement.dat").exists() ? "existe" : "no existe"));
		reset();
	}
